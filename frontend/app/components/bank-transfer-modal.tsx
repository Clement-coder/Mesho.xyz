...
export function BankTransferModal({
  ...
  return (
    <>
      ...
      {/* Modal — centred, scrollable on small screens */}
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 overflow-y-auto">
        <div
          className="relative bg-background border border-border rounded-2xl w-full max-w-md shadow-2xl my-auto animate-in zoom-in-95 duration-200"
        ...
        >
          ...
          <div className="p-5 space-y-4">
            ...
            <div className={`flex items-center justify-between rounded-xl px-4 py-3 border ${expired ? 'bg-destructive/10 border-destructive/30' : 'bg-accent/10 border-accent/20'}`}>
              ...
              <span className={`font-mono font-bold text-xl tabular-nums ${expired ? 'text-destructive' : 'text-accent'}`}>
                {mins}:{secs}
              </span>
            </div>
            ...
            {expired && (
              <div className="flex items-start gap-2 text-sm text-destructive bg-destructive/5 border border-destructive/20 rounded-xl p-3">
                ...
                <span>Session expired. Close this modal and click <strong>Get This Material</strong> again to get a fresh reference.</span>
              </div>
            )}
            ...
            <div className="space-y-2">
              ...
              {details.map(item => (
                <div
                  key={item.label}
                  className="flex items-center justify-between bg-muted/40 rounded-xl px-4 py-2.5 gap-3"
                >
                  ...
                  {item.copyable && (
                    <button
                      ...
                      className="flex-shrink-0 p-1.5 hover:bg-muted rounded-lg transition-colors text-muted-foreground hover:text-foreground"
            ...
            <div className="bg-muted/30 rounded-xl p-3 text-xs text-muted-foreground leading-relaxed">
              ⚠️ Use the <strong className="text-foreground">reference number</strong> as your transfer narration/description.
              After sending the money, tap <strong className="text-foreground">"I Have Paid"</strong> — the admin will verify
              and send your material via WhatsApp or email.
            </div>
